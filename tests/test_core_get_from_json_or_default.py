...
class TestCoreGetFromJsonOrDefault:
    ...
    def test_noneinput(self) -> None:  # noqa: D102
        value = get_from_json_or_default(None, None, None)
        assert not value

    def test_noneinputwithemptybreadcrumb(self) -> None:  # noqa: D102
        value = get_from_json_or_default(None, '', None)
        assert not value

    def test_noneinputwithbreadcrumb(self) -> None:  # noqa: D102
        value = get_from_json_or_default(None, 'test', None)
        assert not value

    def test_noneinputwithbreadcrumbcustom(self) -> None:  # noqa: D102
        value = get_from_json_or_default(None, 'test', 'custom')
        assert value == 'custom'

    def test_firstlevelbreadcrumb(self) -> None:  # noqa: D102
        value = get_from_json_or_default({
            'test': 'response'
        }, 'test', 'custom')
        assert value == 'response'

    def test_secondlevelbreadcrumb(self) -> None:  # noqa: D102
        value = get_from_json_or_default({
            'test': {
                'subtest': 'response'
            }
    ...
    def test_secondlevelbreadcrumbmiss(self) -> None:  # noqa: D102
        value = get_from_json_or_default({
            'test': {
                'subtest': 'response'
            }
