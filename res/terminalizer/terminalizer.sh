...
# For recording we use a dockerized mint pype-cli inside zsh
if [ "$1" == "record" ]; then
    ...
    docker cp $cwd/docker-entrypoint.sh $\
    cnt_name-container:/usr/local/bin/docker-entrypoint.sh
...
if [ "$1" == "render" ]; then
    ...
    [ "$1" == "render" ] && $bin render record.yml \
    --output pype-cli.gif --quality 100
fi
